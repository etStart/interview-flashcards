...
export default function CloudPanel(props: CloudPanelProps) {
  ...
  return (
    <div className="modal-shell" role="presentation">
      <div className="modal-card" role="dialog" aria-modal="true" aria-label="账号与同步">
        ...
        {!syncInfo.configured ? (
          <div className="cloud-block">
            <p>请先在 Supabase 中配置 `.env.local` 里的这两个变量：</p>
          ...
          <div className="cloud-block">
            <div className="auth-switcher" role="tablist" aria-label="登录方式">
              <button
            ...
            <p>{mode === "signup" ? "创建账号后即可把当前学习进度同步到云端。" : "登录后会自动合并本地和云端进度。"}</p>
            <input
            ...
            <p className="modal-message">如果注册后没有自动登录，请检查 Supabase Dashboard 里的 `Confirm email`。</p>

            <div className="modal-actions">
              <button className="primary-button" disabled={isBusy} onClick={handleAuthSubmit}>
                {isBusy ? "提交中..." : mode === "signup" ? "注册并登录" : "登录"}
              </button>
            </div>
          </div>
        )}
